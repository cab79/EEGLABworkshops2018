...
if S.combinefiles
    ...
    for i = 1:length(S.([S.fnameparts{end} 's']))
        S.([S.fnameparts{end} 's']){i} = strrep(S.([S.fnameparts{end} 's']){i},'.','');
    ...
    for s = 1:length(S.subjects)
        ...
        % SAVE
        sname_ext = 'combined';
        sname = [S.subjects{s} '_' sname_ext '.' S.loadext];
        EEG = pop_saveset(EEG,'filename',sname,'filepath',S.setpath);
    end
end
